...
class TestBuildLoopDoMode:
    ...
    @patch("devflow.orchestration.execution._execute_phase")
    ...
    @patch("devflow.orchestration.phase_handlers.setup_gate_retry", return_value=False)
    ...
    def test_failure_keeps_changes_on_branch(
        ...
        # Calls: initial SHA save, pre-phase SHA (implementing), pre-phase
        # SHA (gate — captured up front for every phase now), failure-path
        # SHA check.
