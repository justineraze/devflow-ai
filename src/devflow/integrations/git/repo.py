...
def create_branch(feature_id: str) -> str:
    ...
    if result.returncode != 0:
        ...
        if "already exists" in result.stderr:
            ...
            raise RuntimeError(
                f"✗ Branch creation failed — git checkout -b {branch}: {result.stderr.strip()}"
                " — Fix: check for naming conflicts or stash uncommitted changes"
            )
    return branch
...
def get_diff(cached: bool = False) -> str:
    """Return the full text of ``git diff`` (working tree or staged)."""
    args = ["diff"]
    if cached:
        args.append("--cached")
    result = _git(*args)
